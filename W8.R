#1
devicepref = read.csv("/Users/joeko/Joe's/NCKU/2024Fall/Coursera/Designing-Running-and-Analyzing-Experiments/materials/deviceprefsSR.csv") 
View(devicepref)


#2. Use binomial regression to examine Pref by Disability and Senior. 
...
m = glm(Pref ~ Disability*Senior, data=devicepref, family=binomial)
Anova(m, type=3)
# p= 0.403997  
...
contrasts(devicepref$Senior) <- "contr.sum"
contrasts(devicepref$Disability) <- "contr.sum"
m = multinom(Pref ~ Disability*Senior, data=devicepref) # multinomial logistic
Anova(m, type=3)
#p= 0.404087 

#4. whether there was a significant preference for touchpads or trackballs within each Disability × Senior combination. Conduct such an exploration using post hoc binomial tests.
trackball.m0 = binom.test(sum(devicepref[devicepref$Senior == "M" & devicepref$Disability == "0",]$Pref == "trackball"), nrow(devicepref[devicepref$Senior == "M" & devicepref$Disability == "0",]), p=1/2)
trackball.m1 = binom.test(sum(devicepref[devicepref$Senior == "M" & devicepref$Disability == "1",]$Pref == "trackball"), nrow(devicepref[devicepref$Senior == "M" & devicepref$Disability == "1",]), p=1/2)
touchpad.f0 = binom.test(sum(devicepref[devicepref$Senior == "F" & devicepref$Disability == "0",]$Pref == "touchpad"), nrow(devicepref[devicepref$Senior == "F" & devicepref$Disability == "0",]), p=1/2)
touchpad.f1 = binom.test(sum(devicepref[devicepref$Senior == "F" & devicepref$Disability == "1",]$Pref == "touchpad"), nrow(devicepref[devicepref$Senior == "F" & devicepref$Disability == "1",]), p=1/2)
...
flightbooking = read.csv("/Users/joeko/Joe's/NCKU/2024Fall/Coursera/Designing-Running-and-Analyzing-Experiments/materials/bookflights.csv") 
View(flightbooking)
...
# Calculate pairwise least-squares means
pairwise_results <- lsmeans(m, pairwise ~ Website * International)

# Convert pairwise comparisons to a glht object and summarize
summary(as.glht(pairs(pairwise_results)), test = adjusted(type = "none"))
