...
designide = read.csv("/Users/joeko/Joe's/NCKU/2024Fall/Coursera/Designing-Running-and-Analyzing-Experiments/materials/designtime.csv")
View(designide)
...
# Mann-Whitney U test
library(coin)
# Convert 'Tool' to a factor if it's a character
designide$Tool <- as.factor(designide$Tool)

# Convert 'Time' and 'logTime' to numeric if they are not already
designide$Time <- as.numeric(designide$Time)
designide$logTime <- as.numeric(designide$logTime)
# Run the test with 'Time'
wilcox_test(Time ~ Tool, data = designide, distribution = "exact")

# Run the test with 'logTime'
wilcox_test(logTime ~ Tool, data = designide, distribution = "exact")
